...
public struct RawSlideElement: Equatable {
    ...
    /// prefix used anywhere inside the subtree is re-declared as an
    /// `xmlns:` attribute on this string's own root element, so it stays
    /// well-formed when spliced verbatim into the output slide XML —
    /// regardless of what that slide's own root happens to declare.
    /// `XMLElement.xmlString` on a node detached from its parsed document
    /// does *not* re-declare namespaces the node only inherited from an
    /// ancestor (verified empirically; not documented behavior), so a plain
    /// `element.xmlString` here would silently produce a fragment with
    /// unbound prefixes such as `mc:` the moment it left the document that
    ...
    /// `PptxWriter.validateSupportedContent`.
